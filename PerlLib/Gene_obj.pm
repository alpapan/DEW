...
sub to_GFF3_format {
    ...
    $gff3_text .= "$asmbl_id\t$source\tgene\t$gene_lend\t$gene_rend\t.\t$strand\t.\tID=$gene_id;Name=$com_name;$gene_alias\n";
...
sub to_BED_format {
	...
	if ($self->{identity}){
		$com_name.= ';identity='.$self->{identity};
        }

	if ($self->{coverage}){
		$com_name.= ';coverage='.$self->{coverage};
        }
