...
sub index_GFF3_gene_objs {
    ...
    my %secondary_attributes; #secondary gff data
    ...
    while (<$fh>) {
        ...
        if ($feat_type eq 'mRNA') {
            ...
            # other lowercase attributes of interest (e.g.gmap)
            while ($gene_info =~/;([a-z]+)=([^;\s]+)/g){
	       $secondary_attributes{$parent}{$1} = $2 if $1 && $2;
            }
            next;
        }
    ...
    foreach my $asmbl_id (sort keys %gene_coords) {
		...
		foreach my $gene_id (keys %$genes_href) {
            ...
            foreach my $transcript_id (keys %$transcripts_href) {
                ...
                if (exists $secondary_attributes{$gene_id}){
                    foreach my $key (keys %{$secondary_attributes{$gene_id}}){
                        $gene_obj->{$key} = $secondary_attributes{$gene_id}{$key};
                    }
                }
