...
die "No differential expression found"
  unless -s $fpkm_expression_level_matrix_TMM;
...
sub sqlite_backup() {
  ...
  if ( -s $backup_db_file . ".tmp" ) {
    ...
    print "Checkpointing database. Do not halt... SQL database checkpointed!\n";
  } else {
    print
...
# due to kangade only one aln file so we have re-align if it has not been tested for a readset
    ...
    for ( my $i = 0 ; $i < (@readsets) ; $i++ ) {
      ...
      while ( my $seq_obj = $file_obj->next_seq() ) {
        my $id          = $seq_obj->id();
        my $seq_md5hash = &sqlite_get_md5($id);
        my $aln_exists  = &sqlite_get_depth_data( $seq_md5hash, $readsets[$i] );
        if ( !$aln_exists ) {
...
sub prepare_alignment_from_existing() {
  ...
  my $baseout = $file_to_align;
  $baseout =~ s/.toalign$//;
  die "Given BAM alignment file does not exist: " . $use_existing_bam[$i] . "\n"
    unless $use_existing_bam[$i] && -s $use_existing_bam[$i];
  my $readset          = $readsets[$i];
    ...
    for ( my $i = 0 ; $i < ( scalar(@readsets) - 1 ) ; $i++ ) {
      ...
      for ( my $k = $i + 1 ; $k < scalar(@readsets) ; $k++ ) {
        ...
        unless ( -s "$out.stats.csv" ) {
          ...
          } elsif ( !$alignment_sam_file_C || !-s $alignment_sam_file_C ) {
          ...
          } elsif ( !$alignment_sam_file_E || !-s $alignment_sam_file_E ) {
...
sub perform_readset_metadata($$) {
  ...
  if ( $read_format eq 'fastq' ) {
    ...
    &process_cmd("$samtools_exec index $readset") unless -s $readset . '.bai';
...
sub process_alignments($) {
  ...
  die "File $alignment_bam cannot be indexed" unless -s $alignment_bam . '.bai';
  ...
  my $alignment_proportion =
    sprintf( "%.2f%%",
             ( $reads_that_aligned / $readset_metadata->{'total_reads'} ) *
               100 );
...
sub perform_stats() {
  ...
  print STATS_RATIO "\tExpress-corrected FPKM\tExpress-corrected counts"
    if $perform_bias_correction;
  print STATS_RATIO "\n";
  print STATS "Checksum\tReadset\tGene alias\tRPKM\tMean\tstd. dev\tMedian\tUpper limit\n";
...
# given an array of genes with expression levels, sort it, and give me a distribution so i can see how it looks.
  ...
  print OUT "Checksum\tGene alias\tReadset\tRPKM\tFPKM\teff.counts\tKANGADE\n";
...
sub prepare_edgeR_graphs() {
  ...
  foreach my $edgeR_result (@edgeR_results) {
    ...
    while ( my $ln = <IN> ) {
      my @data = split( "\t", $ln );
      next
        unless $data[4] || $data[4] > $fdr_pval_cutoff || abs( $data[2] ) < 2;
...
"No differentially expressed transcripts identified at cuttoffs: FDR p-value:$fdr_pval_cutoff, log2(fold-change):2\n\n";
    ...
    if ( $ps2pdf_exec && $pdfcrop_exec ) {
      ...
      foreach my $ps (@ps_files) {
        my $pdf = $ps;
        $pdf =~ s/\.(\w+)$/.pdf/;
        &process_cmd( $ps2pdf_exec . "$pdf.t $ps >/dev/null 2>/dev/null" );
        &process_cmd( $pdfcrop_exec . " $pdf.t $pdf >/dev/null  2>/dev/null" );
        unlink("$pdf.t");
      }
      if ($inkscape_exec) {
...
# N.B. Remember: R/FPKM is for looking at differences between libraries and mean/median_hits for looking at differences within a library. RPKM is appropriate for Single-End libraries and FPKM for Paired-End libraries
...
sub perform_TMM_normalization_edgeR() {
  ...
  foreach my $id (@reference_sequence_list) {
    my $seq_md5hash = &sqlite_get_md5($id);
    $trans_lengths{$seq_md5hash} = &sqlite_get_seq_length($seq_md5hash);
  }
  ...
  foreach my $col (@cols) {
    my $colname = $col;
    $colname =~ s/\W/_/g;
    my $dat_file = $edgeR_dir . "$colname.dat";
    $readset_R_filenames{$col} = $dat_file;
  ...
  undef(@cols);
  @cols = keys %readset_R_filenames;
  &run_TMM( \%readset_R_filenames, $TMM_file );
  unless ( -s $TMM_file ) {
    warn "TMM normalization failed. Skipping\n";
    print LOG "TMM normalization failed. Skipping\n";
    return;
  }
  $fpkm_TMM_matrix_file =
    &write_normalized_fpkm_file( $matrix_file, $TMM_file, \%trans_lengths );
  unless ( $fpkm_TMM_matrix_file && -s $fpkm_TMM_matrix_file ) {
    warn "EdgeR failed to produce the output file\n";
    warn $fpkm_TMM_matrix_file . "\n" if $fpkm_TMM_matrix_file;
    return;
  }
  unlink( $matrix_file . '.normalized.FPKM' );
  link( $fpkm_TMM_matrix_file, $matrix_file . '.normalized.FPKM' );
  for ( my $i = 0 ; $i < @cols - 1 ; $i++ ) {
    for ( my $k = $i + 1 ; $k < @cols ; $k++ ) {

   # if we have replicates, dispersion can be set to auto. otherwise set it to 0.1
      &run_edgeR( $cols[$i], $cols[$k], \%readset_R_filenames, $edgeR_dispersion,
                  $fdr_pval_cutoff );
    }
  }
...
<body onload="show_heatmap();">
  ...
  close HTML;
  mkdir( $js_dir . '/js' );
  link( $RealBin . "/js/canvasXpress.min.js",
...
sub run_TMM {
  ...
  foreach my $name ( keys %{$readset_R_filenames_href} ) {
    ...
    print OUT1 join( "\t", $R_filename, $library_metadata{$name}{'group' }, "$name sample" ) . "\n";
  }
  close OUT1;
...
sub run_edgeR {
  ...
  # accounting for groups (column 2 [1] in target.dat and TMM_info.txt files)
  my ( $col_A, $col_B, $filenames_href, $dispersion, $FDR ) = @_;
  my $base        = $edgeR_dir . $col_A . '_vs_' . $col_B . ".edgeR";
  my $R_script    = $base . "_run.R";
  my $target_file = $base . "_target.dat";
  open( OUT1, ">$target_file" )
    or die "Error, cannot write to $target_file: $!";
  print OUT1 "files\tgroup\tdescription\n";
  print OUT1 join( "\t", $filenames_href->{$col_A}, $col_A, "$col_A sample" )
    . "\n";
  print OUT1 join( "\t", $filenames_href->{$col_B}, $col_B, "$col_B sample" )
    . "\n";
  close OUT1;
  open( OUTR, ">$R_script" ) or die "Error, cannot write $R_script: $!";
  print OUTR "
  source('$RealBin/R/edgeR_funcs.R')
  edgeR_DE_analysis(targetsFile='$target_file',datafile='$base' ,dispersion=$dispersion,FDR=$FDR,kclusters=$tree_clusters);
  ";
  close OUTR;
  &process_cmd( "R --vanilla --slave -f $R_script >/dev/null 2>/dev/null",
                $edgeR_dir );
...
sub write_normalized_fpkm_file {
  ...
  if ( scalar(@headers) < 8 ) {
    open( STATSOUT, ">$result_dir/$uid.expression_levels.stats.tsv.t" )
      || die($!);
    chomp($headerS);
    print STATSOUT $headerS . "\tTMM FPKM\n";
    while ( my $ln = <STATSIN> ) {
      chomp($ln);
      my @data = split( "\t", $ln );
      next unless $data[2];
      print STATSOUT $ln . "\t" . $hash{ $data[0] }{ $data[2] } . "\n";
    }
    close STATSIN;
    close STATSOUT;
  ...
  if ( scalar(@headers) < 9 ) {
    ...
    while ( my $ln = <STATSIN> ) {
      ...
      next unless $data[2];
      my $ratio =
        $hash{ $data[0] }{ $data[3] } && $hash{ $data[0] }{ $data[3] } > 0
        ? sprintf( "%.2f",
                   $hash{ $data[0] }{ $data[2] } /
                     $hash{ $data[0] }{ $data[3] } )
        : int(0);
