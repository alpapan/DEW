...
namespace FilterReadsByDepth
{
    ...
    class Program
    {
        ...
        static StreamWriter[] filteredReadsMin;
        ...
        static void Main(string[] args)
        {
            ...
            filteredReadsMin = new StreamWriter[2];
            ...
            for (int f = 0; f < noOfReadsFiles; f += 2)
            {
                ...
                for (int p = 0; p < 2; p++)
                {
                    if (f + p < noOfReadsFiles)
                    {
                        ...
                        if (!histoOnly)
                        {
                            ...
                            filteredReadsMin[p] = new StreamWriter(outputPath + fileWithoutSuffix + "_below_" + minDepth + fileSuffix,
                                                                false, readsFiles[p].CurrentEncoding, 1000000);
                            filteredReadsMin[p].NewLine = lfConvention;
                        }
                    }
                    else
                    {
                ...
                for (int b = 0; b < noThreads; b++)
                {
                    ...
                    filterParams[b].filteredReadsMin = filteredReadsMin;
                ...
                foreach (StreamWriter r in filteredReads)
                    if (r != null)
                        r.Close();
                foreach (StreamWriter r in filteredReadsMin)
                    if (r != null)
                        r.Close();
        ...
        private static void FilteringThread(object threadParams)
        {
            ...
            StreamWriter[] filteredReadsMin = theseParams.filteredReadsMin;   // corresponding (shared) streams for filtered reads below min
                            ...
                            for (int f = 0; f < noReadFNs; f++)
                                if (readValid[b, f])
                                {
                                    if (keepThisReadSet[b])
                                    {
                                        ...
                                        SaveFilteredReadAndQualMin(filteredReadsMin[f], readHeaderSet[b, f], readSet[b, f], qualsSet[b, f]);
        ...
        private static void SaveFilteredReadAndQualMin(StreamWriter filteredReadsMin, string header, string filteredReadMin, string quals)
        {
            MerStrings.WriteRead(filteredReadsMin, header, filteredReadMin, readsFormat);
            if (readsFormat == MerStrings.formatFASTQ)
            {
                if (fullQualHeaders)
                    filteredReadsMin.WriteLine("+" + header.Substring(1));
                else
                    filteredReadsMin.WriteLine("+");
                filteredReadsMin.WriteLine(quals);
            }
        }
    ...
    public class filterThreadParams
    {
        ...
        public StreamWriter[] filteredReadsMin;
